...
?>data-catalog-template="<?= $arParams["TEMPLATE_AJAX_ID"] ?>"><?
             ...
             if ($arParams['ALFA_SORT_BY_SHOW'] == 'Y') {
                 ...
                 ?><span class="title hidden-xs hidden-sm"><?= GetMessage('RS.MONOPOLY.SORT_TITLE') ?></span><?
                 ?><div class="dropdown"><?
                 ?><button class="dropdown-toggle" type="button" id="dropdownMenuSortBy" data-toggle="dropdown" aria-expanded="true"><?
        ...
        if ($arParams['ALFA_OUTPUT_OF_SHOW'] == 'Y') {
            ...
            ?><div class="dropdown"><?
            ?><button class="dropdown-toggle" type="button" id="dropdownMenuOutput" data-toggle="dropdown" aria-expanded="true"><?
            ?><i class="fa visible-xs-inline fileicon"></i><?= ($arResult['USING']['COUTPUT']['ARRAY']['VALUE'] == "2000") ? 'Все' : $arResult['USING']['COUTPUT']['ARRAY']['VALUE'] ?><i class="fa hidden-xs value arrowright"></i><?
            ?></button><?
            ?><ul class="dropdown-menu list-unstyled" role="menu" aria-labelledby="dropdownMenuOutput"><?
                    foreach ($arResult['COUTPUT'] as $output) {
                        ?><li><a href="<?= $output['URL'] ?>"><?= ($output['VALUE'] == "2000") ? 'Все' : $output['VALUE'] ?></a></li><?
                        }
                        ?></ul><?
                        ?></div><?
                        ?></div><?
        ...
        if ($arParams['ALFA_CHOSE_TEMPLATES_SHOW'] == 'Y') {
            ...
            ?><span class="title hidden-xs hidden-sm"><?= GetMessage('RS.MONOPOLY.SHOW_TITLE') ?></span><?
            ?><div class="dropdown"><?
                foreach ($arResult['CTEMPLATE'] as $template) {
                    ?><a class="fd_<?= $template['VALUE'] ?><? if ($template['USING'] == 'Y'): ?> selected<? endif; ?>" href="<?= $template['URL'] ?>"><i class="fa <?= $template['VALUE'] ?>"></i></a><?
                    }
                    ?></div><?
                    ?></div><?
                ...
                foreach ($arResult['CTEMPLATE'] as $template) {
                    ?><li class="fd_<?= $template['VALUE'] ?>"><a href="<?= $template['URL'] ?>"><?
                    ?><i class="fa <?= $template['VALUE'] ?>"></i><?
                    ?></a></li><?
                    }
                    ?></ul><?
