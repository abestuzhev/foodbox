...
{
	/* var buyMore = new BX.PopupWindow(
			 "grafikRaboty",                
			  null, 
			 {
				content: "<div id='buyMore'>Бесплатная доставка от 1500 руб.<br>(в пределах МКАД)<br>Для бесплатной доставки Вам не хватает:<br>"+(freeDeliverySum -curSum)+"  руб.</div>",
				autoHide: false,
				offsetLeft: 0,
				offsetTop: 0,
				overlay : true,
	...
	var buttons = [						
		new BX.PopupWindowButton({
			...
			events: {
				click: function() {location.href='/catalog/'}
			}
		}),
		new BX.PopupWindowButton({
			ownerClass: "btn",
			...
			events: {
				click: function() {AddToBasketPopup.close()}
			}
		})
	];
