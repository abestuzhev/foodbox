...
<a name="tb"></a>
<a class="aprimary" href="<?=$arParams["PATH_TO_LIST"]?>"><?=GetMessage("SPPD_RECORDS_LIST")?></a>
...
<?if(strlen($arResult["ID"])>0):?>
	...
	<table class="table table-stripped sale_personal_profile_detail data-table">
		<tr>
			<th colspan="2">
		...
		<tr>
			...
			<td width="60%"><input class="form-control" type="text" name="NAME" value="<?echo $arResult["NAME"];?>" size="40"></td>
		</tr>
		<?
					...
					<tr>
						...
						<td width="50%">
							...
							<?elseif ($vval["TYPE"]=="TEXT"):?>
								<input class="form-control" type="text" size="<?echo (IntVal($vval["SIZE1"])>0)?$vval["SIZE1"]:30; ?>" maxlength="250" value="<?echo (isset($currentValue)) ? $currentValue : $vval["DEFAULT_VALUE"];?>" name="<?=$name?>">
							<?elseif ($vval["TYPE"]=="SELECT"):?>
								<select class="form-control" name="<?=$name?>" size="<?echo (IntVal($vval["SIZE1"])>0)?$vval["SIZE1"]:1; ?>">
							...
							<?elseif ($vval["TYPE"]=="MULTISELECT"):?>
								<select class="form-control" multiple name="<?=$name?>[]" size="<?echo (IntVal($vval["SIZE1"])>0)?$vval["SIZE1"]:5; ?>">
									<?
									$arCurVal = array();
							...
							<?elseif ($vval["TYPE"]=="TEXTAREA"):?>
								<textarea class="form-control" rows="<?echo (IntVal($vval["SIZE2"])>0)?$vval["SIZE2"]:4; ?>" cols="<?echo (IntVal($vval["SIZE1"])>0)?$vval["SIZE1"]:40; ?>" name="<?=$name?>"><?echo (isset($currentValue)) ? $currentValue : $vval["DEFAULT_VALUE"];?></textarea>
	...
	<br />
	<div>
		<input class="btn btn-primary" type="submit" name="save" value="<?echo GetMessage("SALE_SAVE") ?>">
		&nbsp;
		<input class="btn btn-default" type="submit" name="apply" value="<?=GetMessage("SALE_APPLY")?>">
		&nbsp;
		<input class="btn btn-default" type="submit" name="reset" value="<?echo GetMessage("SALE_RESET")?>">
	</div>
	</form>
<?else:?>
	<? ShowError($arResult["ERROR_MESSAGE"]);?>
