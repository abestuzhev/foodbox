...
<div class="bx_my_order_switch">
	<a class="bx_mo_link aprimary" href="<?=$arResult["URL_TO_LIST"]?>"><?=GetMessage('SPOD_CUR_ORDERS')?></a>
</div>
<br />
...
<?else:?>
	...
	<table class="table bx_order_list_table">
		<thead>
			<tr>
				<td colspan="2">
				...
				<tr>
					...
					<td>
						...
						<?elseif($arResult["CAN_CANCEL"] == "Y"):?>
							<?=GetMessage('SPOD_NO')?>&nbsp;&nbsp;&nbsp;<a class="btn btn-default" href="<?=$arResult["URL_TO_CANCEL"]?>"><?=GetMessage("SPOD_ORDER_CANCEL")?></a>
						<?endif?>
					</td>
				</tr>
			<?endif?>
	...
	<h3><?=GetMessage('SPOD_ORDER_BASKET')?></h3>
		<div class="table-responsive">
			<table class="table table-striped table-hover bx_order_list_table_order">
				<thead>
					<tr>
				...
				<tbody>
					<?foreach($arResult["BASKET"] as $prod):?>
						<tr>
							...
							<td class="custom img" style="min-width:200px;text-align:center;">
								<?if($hasLink):?>
							...
							<td class="custom"> <span class="fm price text-nowrap"><?=$prod["PRICE_FORMATED"]?></span></td>

							<?if($arResult['HAS_DISCOUNT']):?>
								<td class="custom"> <span class="fm price discount text-nowrap"><?=$prod["DISCOUNT_PRICE_PERCENT_FORMATED"]?></span></td>
							<?endif?>
	...
	<table class="table bx_ordercart_order_sum">
		<tbody>
	...
	<br />
	<a class="aprimary" href="<?=$arResult["URL_TO_LIST"]?>" class="bx_big bx_bt_button_type_2 bx_cart"><?=GetMessage('SPOD_GO_BACK')?></a>
