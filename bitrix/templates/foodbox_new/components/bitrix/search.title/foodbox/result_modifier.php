...
foreach ($arResult["CATEGORIES"] as $category_id => $arCategory) {
    foreach ($arCategory["ITEMS"] as $i => $arItem) {
        if (isset($arItem["ITEM_ID"])) {
            ...
            if (
                    $arItem["MODULE_ID"] == "iblock" && substr($arItem["ITEM_ID"], 0, 1) !== "S"
            ) {
                if ($arCatalogs === false) {
                    $arCatalogs = array();
...
if (!empty($arResult["ELEMENTS"]) && CModule::IncludeModule("iblock")) {
    ...
    while ($arElement = $rsElements->Fetch()) {
        ...
        $dbPic = CIBlockElement::GetByID($arElement["ID"])->Fetch();
        $file = CFile::ResizeImageGet($dbPic["DETAIL_PICTURE"], array('width' => 190, 'height' => 190), BX_RESIZE_IMAGE_PROPORTIONAL, true);
        $arElement['POPUP_PIC'] = $file['src'];
