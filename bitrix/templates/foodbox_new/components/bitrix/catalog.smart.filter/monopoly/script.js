...
JCSmartFilter.prototype.postHandler = function (result, fromCache)
{
	...
	if (!!result && !!result.ITEMS)
	{
		...
		if (!!modef && !!modef_num)
		{
			...
			result.COMPONENT_CONTAINER_ID = "js-ajaxcatalog";
			...
			if (result.INSTANT_RELOAD && result.COMPONENT_CONTAINER_ID)
			{
				...
				RSMONOPOLY_Area2Darken($container);
				BX.ajax({
					url: url,
					method: 'POST',
					dataType: 'json',
					...
					onsuccess: function(json) {
						...
						$container.html(json[result.COMPONENT_CONTAINER_ID]);
						$("#" + result.COMPONENT_CONTAINER_ID + "_sorter").html(
							json[result.COMPONENT_CONTAINER_ID + "_sorter"]
						);
						RSMONOPOLY_Area2Darken($container);
                        smartFilter.ftype1ShowOnSM();
					},
					onfailure: function() {
						RSMONOPOLY_Area2Darken();
					}
				});
			}
			else
			{
				if($(document).width()>993) {
					...
					if (this.viewMode != "ftype2")
					{
						...
						//curProp = ;
						//console.log(BX.findChild(BX.findParent(this.curFilterinput, {'class':'bx_filter_parameters_box'}), {'class':'bx_filter_container_modef'}, true, true).length);
						if (BX.findChild(BX.findParent(this.curFilterinput, {'class':'bx_filter_parameters_box'}), {'class':'bx_filter_container_modef'}, true, true).length == 1) {
							curProp = BX.findChild(BX.findParent(this.curFilterinput, {'class':'bx_filter_parameters_box'}), {'class':'bx_filter_container_modef'}, true, false);
							
						} else  {
							curProp = BX.findChild(BX.findParent(this.curFilterinput, {'class':'bx_filter_input_checkbox'}), {'class':'bx_filter_container_modef'}, true, false);
