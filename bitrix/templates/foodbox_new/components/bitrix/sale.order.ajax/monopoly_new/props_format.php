    ...
    function PrintPropsForm($arSource = array(), $locationTemplate = ".default") {
        ...
        if (!empty($arSource)) {
            ...
            foreach ($arSource as $arProperties) {
                ...
                ?>
                <div class = "col col-md-10 prop_<?= $arProperties["CODE"] ?>" data-property-id-row="<?= intval(intval($arProperties["ID"])) ?>">
                    ...
                    elseif ($arProperties["TYPE"] == "TEXT") {
                        ...
                        if($arProperties["FIELD_NAME"]=="ORDER_PROP_7")
                        {
                            ...
                            if($USER->IsAuthorized())
                            {
                                ...
                                $arUser = $rsUser->Fetch();
                                $arProperties["VALUE"] = $arUser["PERSONAL_CITY"];
                            }
                        }
                        ?>
                        <div class = "form-group">
                            ...
                            <? if ($arProperties["REQUIED_FORMATED"] == "Y"): ?>
                                <span class="required">*</span>
                            <? endif; ?>
                            <input class = "form-control" type="text" <? //= (in_array($arProperties["ID"], Array(1, 3, 7)) ? "required" : "")  ?> maxlength="250" size="<?= $arProperties["SIZE1"] ?>" 
                            value="<?= $arProperties['VALUE']?>" name="<?= $arProperties["FIELD_NAME"] ?>" id="<?= $arProperties["FIELD_NAME"] ?>" <? if (strlen(trim($arProperties["DESCRIPTION"])) > 0): ?>placeholder="<?= $arProperties["DESCRIPTION"] ?>"<? endif; ?> />
                            <? /* if (strlen(trim($arProperties["DESCRIPTION"])) > 0):?>
                              <span id="helpBlock" class="help-block">
                              <?=$arProperties["DESCRIPTION"]?>
                              </span>
