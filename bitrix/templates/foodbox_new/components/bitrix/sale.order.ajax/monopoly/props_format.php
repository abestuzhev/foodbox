		...
		if (!empty($arSource))
		{
			?>
					...
					foreach ($arSource as $arProperties)
					{
						...
						?>
						<div class = "col col-md-6 prop_<?=$arProperties["CODE"]?>" data-property-id-row="<?=intval(intval($arProperties["ID"]))?>">
						...
						elseif ($arProperties["TYPE"] == "TEXT")
						{
							...
							<div class = "form-group">
								...
								<?if ($arProperties["REQUIED_FORMATED"]=="Y"):?>
									<span class="required">*</span>
								<?endif;?>
								<input class = "form-control" type="text" maxlength="250" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" <? if (strlen(trim($arProperties["DESCRIPTION"])) > 0):?>placeholder="<?=$arProperties["DESCRIPTION"]?>"<? endif; ?> />
								<?/* if (strlen(trim($arProperties["DESCRIPTION"])) > 0):?>
									<span id="helpBlock" class="help-block">
										<?=$arProperties["DESCRIPTION"]?>
									</span>
