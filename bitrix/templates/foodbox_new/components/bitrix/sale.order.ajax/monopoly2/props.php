	...
	if (is_array($arResult["ORDER_PROP"]["USER_PROFILES"]) && !empty($arResult["ORDER_PROP"]["USER_PROFILES"])):
		...
		?>
			<div class=form-group">
				<label for="ID_PROFILE_ID" class="col-sm-3 control-label text-nowrap"><?=GetMessage("SOA_TEMPL_PROP_CHOOSE")?></label>
				<div class="col-sm-9">
					<select class="form-control" name="PROFILE_ID" id="ID_PROFILE_ID" onChange="SetContact(this.value)">
						<option value="0"><?=GetMessage("SOA_TEMPL_PROP_NEW_PROFILE")?></option>
						<?
						foreach($arResult["ORDER_PROP"]["USER_PROFILES"] as $arUserProfiles)
						{
							?>
							<option class="form-control" value="<?= $arUserProfiles["ID"] ?>"<?if ($arUserProfiles["CHECKED"]=="Y") echo " selected";?>><?=$arUserProfiles["NAME"]?></option>
							<?
						}
						?>
					</select>
		...
		?>
			<div class=form-group">
				<label for="ID_PROFILE_ID" class="col-sm-3 control-label text-nowrap"><?=GetMessage("SOA_TEMPL_EXISTING_PROFILE")?></label>
				<div class="col-sm-9">
					<?
...
<div class="bx_section">
	<h4>
		...
		?>
			<a href="#" class="slide small aprimary" onclick="fGetBuyerProps(this); return false;">
