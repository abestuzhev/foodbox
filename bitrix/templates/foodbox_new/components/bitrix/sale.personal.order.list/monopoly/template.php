...
<?else:?>
	...
	<div class="bx_my_order_switch">
		...
		<?if($nothing || isset($_REQUEST["filter_history"])):?>
			<a class="bx_mo_link btn btn-default" href="<?=$arResult["CURRENT_PAGE"]?>?show_all=Y"><?=GetMessage('SPOL_ORDERS_ALL')?></a>
		<?endif?>

		<?if($_REQUEST["filter_history"] == 'Y' || $_REQUEST["show_all"] == 'Y'):?>
			<a class="bx_mo_link btn btn-default" href="<?=$arResult["CURRENT_PAGE"]?>?filter_history=N"><?=GetMessage('SPOL_CUR_ORDERS')?></a>
		<?endif?>

		<?if($nothing || $_REQUEST["filter_history"] == 'N' || $_REQUEST["show_all"] == 'Y'):?>
			<a class="bx_mo_link btn btn-default" href="<?=$arResult["CURRENT_PAGE"]?>?filter_history=Y"><?=GetMessage('SPOL_ORDERS_HISTORY')?></a>
		<?endif?>
		...
		<?foreach($arResult["ORDER_BY_STATUS"] as $key => $group):?>
			...
			<?foreach($group as $k => $order):?>
				...
				<div class="bx_my_order">
					...
					<table class="table table-stripped bx_my_order_table">
						<thead>
							<tr>
								<td>
									<a class="aprimary" href="<?=$order["ORDER"]["URL_TO_DETAIL"]?>">
								...
								<td style="text-align: right;">
									<a class="aprimary" href="<?=$order["ORDER"]["URL_TO_DETAIL"]?>"><?=GetMessage('SPOL_ORDER_DETAIL')?></a>
								</td>
							</tr>
						</thead>
						<tbody>
							<tr>
								...
								<td>
									...
									if($arResult["INFO"]["STATUS"][$key]['COLOR']=='yellow'){
										$CSSClass = 'label label-warning';
									} elseif($arResult["INFO"]["STATUS"][$key]['COLOR']=='red'){
										$CSSClass = 'label label-danger';
									} elseif($arResult["INFO"]["STATUS"][$key]['COLOR']=='gray'){
										$CSSClass = 'label label-info';
									}
									?>
									<div><span class="bx_my_order_status <?=$CSSClass?> <?=$arResult["INFO"]["STATUS"][$key]['COLOR']?><?/*yellow*/ /*red*/ /*green*/ /*gray*/?>"><?=$arResult["INFO"]["STATUS"][$key]["NAME"]?></span></div>
									<br />
									<?if($order["ORDER"]["CANCELED"] != "Y"):?>
										<a class="btn btn-default" href="<?=$order["ORDER"]["URL_TO_CANCEL"]?>" style="min-width:140px"class="bx_big bx_bt_button_type_2 bx_cart bx_order_action"><?=GetMessage('SPOL_CANCEL_ORDER')?></a>
									<?endif?>

									<a class="btn btn-default" href="<?=$order["ORDER"]["URL_TO_COPY"]?>" style="min-width:140px"class="bx_big bx_bt_button_type_2 bx_cart bx_order_action"><?=GetMessage('SPOL_REPEAT_ORDER')?></a>
								</td>
							</tr>
						</tbody>
					</table>
