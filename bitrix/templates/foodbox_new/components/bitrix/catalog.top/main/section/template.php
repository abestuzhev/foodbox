...
<h2>
<?
 echo GetMessage($arParams['FILTER_NAME']);
?>
</h2>
<div class="bx_catalog_top_home  <? echo $templateData['TEMPLATE_CLASS']; ?>">
<?
...
?>
	<div class="<? echo ($arItem['SECOND_PICT'] ? 'bx_catalog_item ' : 'bx_catalog_item'); ?>"><div class="bx_catalog_item_container" id="<? echo $strMainID; ?>">
...
?>
		...
		{
			echo "Заказ по ".$arItem['CATALOG_MEASURE_RATIO']." шт.";
		}
		?>
		</div>
		</div>
<?
	...
	{
?>
		<div class="bx_catalog_item_controls_old js-element">
<?
		if ($arItem['CAN_BUY'])
		{
			...
			{
			?>
			<div class="bx_catalog_item_controls_blockone add2basketform ">
				<div class="quantity">
					<a id="<? echo $arItemIDs['QUANTITY_DOWN']; ?>" href="javascript:void(0)" class="minus" rel="nofollow"><i class="fa"></i></a><?
					?><input onkeyup="this.value = this.value.replace(/[^\d,.]*/g, '').replace(/,/g, '.').replace(/^[^\d]*(\d+([.,]\d{0,5})?).*$/g, '$1');" type="text" class="js-quantity text-center" id="<? echo $arItemIDs['QUANTITY']; ?>" name="<? echo $arParams["PRODUCT_QUANTITY_VARIABLE"]; ?>" value="<? echo $arItem['CATALOG_MEASURE_RATIO']; ?>"><?
					?><a id="<? echo $arItemIDs['QUANTITY_UP']; ?>" href="javascript:void(0)" class="plus" rel="nofollow"><i class="fa"></i></a><?
					?><span id="<? echo $arItemIDs['QUANTITY_MEASURE']; ?>" class="js-measurename"><? echo $arItem['CATALOG_MEASURE_NAME']; ?></span>
				</div>
			</div>
<?
			}
?>
			...
			<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="bx_catalog_item_controls_blocktwo buybtn" >
				<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="submit" href="javascript:void(0)" rel="nofollow">
<?
