$(function () {
	...
	$(document).on("RSMONOPOLY_fancyBeforeShow", function () {
		var items = [],
			text = '';
	...
	$(".set_add2basket").on("click", function () {
		...
		RSMONOPOLY_Area2Darken($('.constructor-wrapper'));
	...
	$(".my-set .checkbox").on("click", function () {
		...
		if(!isSelected && getSelectedItems().length >= 4) {
			return false;
		}
	...
	function updatePrices(successFn) {
		...
		$.post(url, data, function (result) {
			...
			if(result.formatSum) {
				$panel.find(".price.cool").html(result.formatSum);
			}
			if(result.formatOldSum) {
				$panel.find(".price.old").html(result.formatOldSum);
			}
			if(result.formatDiscDiffSum) {
				$panel.find(".price.discount").html(result.formatDiscDiffSum);
			}
			successFn(result);
		}, "json");
	}
	...
	function addItem($item) {
		...
		$cloneItem.prepend("<div class = 'remove'></div>");
