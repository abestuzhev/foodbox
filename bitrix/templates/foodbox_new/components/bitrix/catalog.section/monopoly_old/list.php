	...
	?><div class="row products <?=$arResult['TEMPLATE_DEFAULT']['CSS']?>"><?
		...
		foreach($arResult['ITEMS'] as $key1 => $arItem) {
			...
			?><div class="item <?
				...
				?>><?
				?><div class="da2_icon hidden-xs"><span><?=GetMessage('DA2_ICON_TITLE')?></span></div><?
				?><div class="qb_icon hidden-xs"><span><?=GetMessage('QB_ICON_TITLE')?></span></div><?
				?><div class="row"><?
												...
												?><div class="buybtn"><?
													?><div><?
														if($HAVE_OFFERS){
															...
															?><a href="<?=$arItem['DETAIL_PAGE_URL']?>" class="btn btn-primary js-morebtn"><?=GetMessage('RS.MONOPOLY.BTN_MORE')?></a><?
															?></form><?
															?></noindex><?
														}
													?></div><?
